...
void Menu::setupUI() {
    ...
    connect(startButton, &QPushButton::clicked, [this](){
        emit this->startGameClicked(4, "hello");
    });
    connect(recordsButton, &QPushButton::clicked, [this](){
        emit this->showRecordsClicked();
    });
    connect(exitButton, &QPushButton::clicked,  [this](){
        emit this->exitGameClicked();
    });
}

void Menu::onStartGame(int size, const QString& playerName) {
    emit startGameClicked(size, playerName);
}
